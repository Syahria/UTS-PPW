...
if(isset($_SESSION["signIn"]) ) {
  header("Location: Dashboardpetugas.php");
  exit;
}
...
if(isset($_POST["signIn"]) ) {
  ...
  $nama = strtolower($_POST["nama_petugas"]);
  $password = $_POST["password"];
  
  $result = mysqli_query($connect, "SELECT * FROM petugas WHERE nama_petugas = '$nama' AND password = '$password' ");
  
  if(mysqli_num_rows($result) === 1) {
    //SET SESSION 
    $_SESSION["signIn"] = true;
    $_SESSION["petugas"]["nama_petugas"] = $nama;
    header("Location: Dashboardpetugas.php");
      exit;
  }
  $error = true;
...
<html lang="en">
  ...
  <div class="col-12">
    ...
    <a class="btn btn-success" href="akseslogin.html">Batal</a>
  </div>
</form>
</div>
<?php if(isset($error)) : ?>
